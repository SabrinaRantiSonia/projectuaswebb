<?php  
	class oop{
		...
		function edit($table,$where){
			...
			$jalan = mysqli_fetch_array(mysql_query($conn,$sql));
			return $jalan;
		}

		function hapus($table,$where){
			$conn = mysqli_connect("localhost","root",'',"listrik_db");
			$sql = mysqli_query($conn,"DELETE FROM $table WHERE $where");
			return $sql;
		}
		...
		function caridata($table){
			$conn = mysqli_connect("localhost","root",'',"listrik_db");
			$sql = mysqli_fetch_array(mysqli_query($conn,"SELECT * FROM $table"));
			return $sql;
		}

		function cekdata($table){
			$conn = mysqli_connect("localhost","root",'',"listrik_db");
			$sql = mysqli_num_rows(mysqli_query($conn,"SELECT * FROM $table"));
			return $sql;
		}		
		...
		function login($table,$username,$password,$alamat){
			@session_start();
			$alamat = "hal_utama.php?menu=home";
			$conn = mysqli_connect("localhost","root",'',"listrik_db");
			$sql = mysqli_query($conn, "SELECT * FROM $table WHERE username like '$username' AND password like '$password'");
