												...
												<?php  
													...
													if ($data=="") {
														...
														<?php  
															if($cek >= 3){
																$no++; 
																$conn = mysqli_connect("localhost","root",'',"listrik_db");																
																$sum = mysqli_fetch_array(mysqli_query($conn,"SELECT id_pelanggan,COUNT(bulan) bln_tunggak,SUM(jumlah_bayar) jml_bayar,SUM(jumlah_meter) jml_meter,tarif_perkwh FROM tagihan WHERE id_pelanggan = '$r[id_pelanggan]' and status = 'Belum Bayar' group by 'id_pelanggan'"));							
																$bulan = mysqli_query($conn,"SELECT * FROM tagihan WHERE id_pelanggan = '$r[id_pelanggan]' AND status = 'Belum Bayar' ");
																?>
																<tr>
																	<td align="center"><?php echo $no; ?>.</td>
																	<td align="center"><?php echo $r['id_pelanggan'] ?></td>
											...
											<tbody>
												<?php  
													...
													if ($data=="") {
														...
														foreach ($data as $r) {
															$no++;
															$penggunaan = $aksi->caridata("penggunaan WHERE id_pelanggan = '$r[id_pelanggan]' AND bulan = '$r[bulan]' AND tahun = '$r[tahun]' group by '$r[id_pelanggan]'");
													?>
														<tr>
															<td align="center"><?php echo $no; ?>.</td>
															<td align="center"><?php echo $r['id_pelanggan']; ?></td>
												...
												<?php } } 
													$conn = mysqli_connect("localhost","root",'',"listrik_db");
													@$sum = mysqli_fetch_array(mysqli_query($conn,"SELECT SUM(jumlah_meter) as meter,SUM(jumlah_bayar) as bayar FROM tagihan WHERE id_pelanggan = '$id_pelanggan' AND tahun = '$tahun'"));
												?>
											</tbody>
											<tfoot>
												<tr>
